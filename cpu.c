...
void cpudetect(Console c)
{
    ...
    if (largeststandardfunction >= 0x01)
    {
        ...
        if (edx & EDX_PSE)      cprint(c, " PSE");
        if (edx & EDX_PAE)      cprint(c, " PAE");
        if (edx & EDX_APIC)     cprint(c, " APIC");
        if (edx & EDX_MTRR)     cprint(c, " MTRR");

        cprint(c, "\n");
        cprint(c, "Instructions:");

        if (edx & EDX_TSC)      cprint(c, " TSC");
        if (edx & EDX_MSR)      cprint(c, " MSR");
        if (edx & EDX_SSE)      cprint(c, " SSE");
        if (edx & EDX_SSE2)     cprint(c, " SSE2");
        if (ecx & ECX_SSE3)     cprint(c, " SSE3");
